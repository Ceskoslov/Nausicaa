...
fn main() -> EvalResult<()> {
    ...
    #[cfg(feature = "live")]
    let config: Option<LiveConfig> = live_config.as_ref().map(|path| -> EvalResult<_> {
        let config: LiveConfig = serde_json::from_slice(&std::fs::read(path)?)?;
        if config.model_snapshot.trim().is_empty() || config.revision.trim().is_empty()
    ...
    let summary = serde_json::json!({
        ...
        "provider_settings": if live_config.is_some() { serde_json::json!({"temperature": 0, "max_completion_tokens": 1024, "timeout_seconds": 30}) } else { serde_json::Value::Null },
